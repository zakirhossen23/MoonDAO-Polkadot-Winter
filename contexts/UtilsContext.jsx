...
export function UtilsProvider({ children }) {
	...
	async function BatchDonate(amount, Recipient, ideas_id,Coin) {
		...
		if (Number(window.ethereum.networkVersion) === 1287 && Coin == "xcvGLMR") { //If Coin is DEV then it will use normal batch
	...
	return (
		<AppContext.Provider value={{ USDPrice: USDPrice, BatchDonate: BatchDonate, BatchJoin: BatchJoin,BatchVoteConviction:BatchVoteConviction, LoadSmartAccount: LoadSmartAccount, getUSDPriceForChain: getUSDPriceForChain, BiconomySmartAccount: BiconomySmartAccount }}>
			{children}
		</AppContext.Provider>
	);
}
